###
#   Original proof of concept crypto.py by an unknown author,
#   referenced from Richard O'Dwyer's mega.py - https://github.com/richardasaurus/mega.py
###
...
def prepare_key(arr):
    ...
    for r in xrange(0x10000):
        for j in xrange(0, len(arr), 4):
            key = [0, 0, 0, 0]
            for i in xrange(4):
                if i + j < len(arr):
                    key[i] = arr[i + j]
